...
annotation_path <- "/data/users/fgribi/transcriptome_assembly/output/MAKER/final/filtered.genes.renamed.final.gff3"

# Load the annotation
annotation <- fread(annotation_path, header = FALSE, sep = "\t")
...
fai_path <- "/data/users/fgribi/transcriptome_assembly/output/samtools/flye.fai"
top20_scaff <- fread(fai_path, header = FALSE, sep = "\t") %>%
    select(V1, V2) %>%
    arrange(desc(V2)) %>%
    head(20)
...
longest_proteins <- "/data/users/fgribi/transcriptome_assembly/output/busco/protein.renamed.longest.fasta"
