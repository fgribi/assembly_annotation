...
dat <- fread("/data/users/fgribi/transcriptome_assembly/output/GENESPACE/genespace/orthofinder/Results_Nov19/Comparative_Genomics_Statistics/Statistics_PerSpecies.tsv", header = T, fill = TRUE)
...
orthogroups <- fread("/data/users/fgribi/transcriptome_assembly/output/GENESPACE/genespace/orthofinder/Results_Nov19/Orthogroups/Orthogroups.GeneCount.tsv")
...
# Orthogroups present in only hiroshima
og_one_species <- ogroups_presence_absence %>%
    filter(SUM == 1) %>%
    filter(hiroshima == TRUE) %>%
    select(Orthogroup) %>%
    pull()

# number of CORE genes in hiroshima
genes_in_og_all_species <- orthogroups %>%
    filter(Orthogroup %in% og_all_species) %>%
    select(hiroshima) %>%
    sum()

# number of SPECIFIC genes in hiroshima
